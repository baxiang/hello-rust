# Rust 教程文档风格指南

> 确保文档质量和一致性

---

## Rust 学习策略（降低学习曲线）

Rust 学习曲线陡峭是业内共识，本教程采用以下策略帮助初学者：

### 1. 分层递进学习

```
┌─────────────────────────────────────────────────────┐
│              Rust 学习金字塔                          │
├─────────────────────────────────────────────────────┤
│                                                     │
│  第一层：基础语法（第 1-6 章）                        │
│  ├── 先熟练 let/fn/if/for                          │
│  ├── 暂时忽略所有权细节                             │
│  └── 目标：能写简单程序                             │
│                                                     │
│  第二层：所有权核心（第 7 章）                       │
│  ├── ★ 最关键章节，需反复练习                       │
│  ├── 理解移动/借用/生命周期                         │
│  ├── 目标：能读懂编译器错误                         │
│                                                     │
│  第三层：数据与抽象（第 12-19 章）                   │
│  ├── 泛型、Trait、闭包、迭代器                      │
│  ├── 目标：能设计类型系统                           │
│                                                     │
│  第四层：实战应用（第 20-31 章）                     │
│  ├── 模块、并发、Web、项目实战                      │
│  ├── 目标：能独立开发项目                           │
│                                                     │
│  原则：每层熟练后再进入下一层                       │
│                                                     │
└─────────────────────────────────────────────────────┘
```

### 2. 场景驱动教学

每个概念必须从"为什么需要"开始：

| 传统写法 | 本教程写法 |
|---------|-----------|
| "所有权是 Rust 的核心特性" | "C++ 程序泄漏内存怎么办？所有权解决这个问题" |
| "Option 表示可能无值" | "Java 的 null 导致 NPE，Rust 用 Option 避免" |
| "&mut 是可变引用" | "函数需要修改参数值，但又不想转移所有权" |

### 3. 对比学习法

与已知语言对比，降低认知负担：

```
┌─────────────────────────────────────────────────────┐
│              语言对比学习                             │
├─────────────────────────────────────────────────────┤
│                                                     │
│  Java/C# 程序员：                                    │
│  ├── null → Option<T>（必须处理无值）              │
│  ├── 引用 → &T（借用，不拥有）                      │
│  ├── 异常 → Result<T, E>（显式处理错误）            │
│                                                     │
│  Python 程序员：                                     │
│  ├── list → Vec<T>（类型固定）                     │
│  ├── dict → HashMap<K, V>                          │
│  ├── 函数返回多值 → 元组/结构体                     │
│                                                     │
│  C/C++ 程序员：                                      │
│  ├── 指针 → 引用（编译器保证安全）                  │
│  ├── 手动管理 → 所有权自动管理                     │
│  ├── 内存泄漏 → 编译器阻止                          │
│                                                     │
│  Go 程序员：                                          │
│  ├── GC → 无 GC（零运行时开销）                     │
│  ├── goroutine → async/task                        │
│  ├── interface → Trait（更严格）                   │
│                                                     │
└─────────────────────────────────────────────────────┘
```

### 4. 编译器当导师

教会读懂错误信息，编译器是最好的老师：

**错误诊断模板（每章必须包含）：**

```markdown
## 常见错误诊断

### 错误类型：使用已移动的值

**错误代码：**
```rust
let s1 = String::from("hello");
let s2 = s1;
println!("{}", s1);  // ❌
```

**编译器信息解读：**
```
error[E0382]: borrow of moved value: `s1`
  │ 说明：s1 已被移动，不能再借用
  │ 提示：编译器告诉你错误原因
```

**修复方案：**
```rust
// 方案 1：克隆
let s2 = s1.clone();

// 方案 2：使用引用
let s2 = &s1;

// 方案 3：返回所有权
let s2 = take_and_return(s1);
```

**关键点：编译器信息是诊断线索，不是障碍**
```

### 5. 错误→修复实战演练

关键章节（特别是第7章）必须包含"错误→修复"演练：

```
┌─────────────────────────────────────────────────────┐
│              错误→修复演练模板                        │
├─────────────────────────────────────────────────────┤
│                                                     │
│  1. 展示错误代码                                    │
│     • 真实的错误场景                                │
│     • 不标注 ❌（让读者自己发现）                   │
│                                                     │
│  2. 运行并观察错误                                  │
│     • 完整的编译器输出                              │
│     • 解读错误信息含义                              │
│                                                     │
│  3. 分析错误原因                                    │
│     • 内存布局变化图                                │
│     • 所有权/借用时间线                             │
│                                                     │
│  4. 提供多种修复方案                                │
│     • 至少 2-3 种解决方案                          │
│     • 对比各方案优劣                                │
│                                                     │
│  5. 强化正确模式                                    │
│     • 总结"如何避免这类错误"                       │
│     • 建立心智模型                                  │
│                                                     │
└─────────────────────────────────────────────────────┘
```

### 6. 检查清单要求

关键章节必须包含"常见错误诊断"部分：

| 章节 | 必须包含的错误诊断 |
|------|------------------|
| 第 7 章 所有权 | 使用已移动的值、悬垂引用 |
| 第 8 章 引用 | 可变/不可变引用冲突 |
| 第 9 章 切片 | 切片越界、生命周期不匹配 |
| 第 17 章 生命周期 | 生命周期标注错误 |
| 第 23 章 并发 | 数据竞争、Send/Sync 错误 |

---

## 核心原则

### 1. 简洁清晰

- 使用简单直接的语言
- 避免冗长复杂的句子
- 一个段落表达一个观点

### 2. 代码优先

- 每个概念都提供代码示例
- 标注正确和错误示例
- 提供运行结果

### 3. 可视化辅助

- 使用 ASCII 艺术绘制图解
- **语法结构图**：每个语法概念必须有结构图
- 展示内存布局和流程
- 增强理解效果

### 4. 互动学习

- 每章包含练习题
- 提供答案和解释
- 鼓励动手实践

---

## Markdown 格式规范

### 标题层级

```markdown
# 章节标题（一级，每个文件只有一个）
> 章节简介（引用块）

## 主要节（二级，主要内容分节）
### 子节（三级，详细内容）
```

**规则：**
- 不使用四级以下标题
- 标题层级不超过三级
- 标题简洁明确

---

### 引用块使用

用于章节开头简介：

```markdown
# 章节名

> 简短介绍章节内容和目标

## 正文内容
```

用于重要提示：

```markdown
> ⚠️ 重要提示：注意事项
```

用于代码说明：

```markdown
> 💡 提示：代码解释
```

---

### 代码块规范

#### Rust 代码

```markdown
```rust
// ✅ 正确示例
fn valid_function() {
    // 正确实现
}

// ❌ 错误示例
fn invalid_function() {
    // 错误实现（会编译失败）
    // 错误信息：cannot find value
}
```
```

#### 输出结果

```markdown
```bash
$ cargo run
Hello, World!
```
```

#### 其他语言

```markdown
```toml
[dependencies]
serde = "1.0"
```

```json
{
  "name": "example"
}
```
```

---

### 列表格式

#### 无序列表

```markdown
- 项目 1
- 项目 2
  - 子项目 2.1
  - 子项目 2.2
- 项目 3
```

#### 有序列表

```markdown
1. 步骤 1
2. 步骤 2
3. 步骤 3
```

#### 任务列表

```markdown
- [x] 已完成
- [ ] 未完成
- [ ] 待添加
```

---

### 表格格式

```markdown
| 列1 | 列2 | 列3 |
|-----|-----|-----|
| 数据1 | 数据2 | 数据3 |
| 数据4 | 数据5 | 数据6 |
```

**对齐方式：**

```markdown
| 左对齐 |居中| 右对齐 |
|:-------|:---:|-------:|
| 内容   | 内容| 内容   |
```

---

### 链接格式

#### 内部链接

```markdown
[章节名](./章节文件.md)
[上级目录](../README.md)
```

#### 外部链接

```markdown
[Rust 官网](https://www.rust-lang.org/)
[文档](https://doc.rust-lang.org/)
```

#### 引用式链接

```markdown
这是[链接文本][ref]。

[ref]: https://example.com
```

---

### 图片格式

```markdown
![图片描述](./images/image.png)
![外部图片](https://example.com/image.png)
```

---

## ASCII 艺术规范

### 框线风格

使用标准 ASCII 字符：

```
┌ ─ ┐  顶部
│   │  侧边
└ ─ ┘  底部
├ ─ ┤  分隔
┬ ┴    连接
```

### 示例

```markdown
\`\`\`
┌─────────────────┐
│  内存布局图     │
├─────────────────┤
│  栈内存         │
│  堆内存         │
└─────────────────┘
\`\`\`
```

### 流程图

```markdown
\`\`\`
流程 A → 流程 B → 流程 C

详细流程：
1. 输入数据
   ↓
2. 处理数据
   ↓
3. 输出结果
\`\`\`
```

---

## 章节结构模板

### 标准章节结构

```markdown
# 章节标题

> 简短介绍（2-3 句）

## 概述

章节主要内容概述。

## 核心概念

### 概念 1

**定义：** [概念定义]

**示例：**

```rust
// 代码示例
```

**应用场景：**
- 场景 1
- 场景 2

### 概念 2

[同上结构]

---

## 最佳实践

### ✅ 推荐

- 实践 1
- 实践 2

### ❌ 避免

- 错误 1
- 错误 2

---

## 常见问题

### 问题 1

**Q: 问题描述？**

**A:** 答案解释。

---

## 小结

本章要点总结：
- 要点 1
- 要点 2
- 要点 3

下一章：[下一章链接]

---

## 练习题

详见：[练习题链接](../../exercises/章节名.md)
```

---

## 代码示例规范

### Rust 代码风格

遵循 AGENTS.md 中的规范：

1. **格式化：** 使用 `cargo fmt`
2. **类型标注：** 公开 API 必须标注
3. **错误处理：** 使用 Result，避免 panic
4. **命名规范：** snake_case 函数，PascalCase 类型

### 示例代码结构

```rust
// ✅ 正确示例
/// 函数说明
fn good_example() {
    // 清晰的代码
}

// ❌ 错误示例
// 错误代码，附带错误信息
fn bad_example() {
    // 错误的实现
}
```

### 输出展示

```markdown
**运行：**

```bash
$ cargo run
[输出结果]
```
```

---

## Rustdoc 文档注释规范

Rust 的文档注释（`///`）支持 doctest，是教程编写的重要规范。

### 文档注释格式

```rust
/// 函数功能简述（一句话）
///
/// 详细说明（可选）：
/// - 参数说明
/// - 返回值说明
/// - 错误情况
///
/// # 示例
///
/// ```
/// let result = add(2, 3);
/// assert_eq!(result, 5);
/// ```
///
/// # 错误
///
/// 如果输入为空，返回 `Err(InvalidInput)`
pub fn add(a: i32, b: i32) -> i32 {
    a + b
}
```

### 示例代码块要求

| 类型 | 格式 | 说明 |
|------|------|------|
| 可测试示例 | ` ``` ` | 无语言标记或 `rust`，会被 doctest 运行 |
| 不可测试 | ` ```rust,ignore ` | 需要 IO、网络等无法在 doctest 运行的场景 |
| 应失败示例 | ` ```rust,should_panic ` | 演示 panic 场景 |
| 编译失败 | ` ```rust,compile_fail ` | 演示编译错误 |

### 教程中的使用

- **第28章（测试与文档）** 必须使用完整 doctest 示例
- **公共 API 章节** 必须包含 `///` 注释示例
- **错误处理章节** 必须展示 `should_panic` 和 `compile_fail` 用法

### 章节注释模板

```rust
/// # 使用示例
///
/// ```
/// use std::collections::HashMap;
///
/// let mut map = HashMap::new();
/// map.insert("key", "value");
/// assert_eq!(map.get("key"), Some(&"value"));
/// ```
```

---

## 常见 Rust 惯用法

教程中的代码示例应优先使用 Rust 惯用写法。

### 模式匹配风格

```rust
// ✅ 推荐：使用 if let 简化单分支
if let Some(value) = option {
    println!("{}", value);
}

// ✅ 推荐：使用 match 处理多分支
match result {
    Ok(value) => println!("{}", value),
    Err(e) => eprintln!("Error: {}", e),
}

// ❌ 避免：过度嵌套
if option.is_some() {
    let value = option.unwrap();
    println!("{}", value);
}
```

### Iterator 链式调用

```rust
// ✅ 推荐：链式调用
let sum: i32 = numbers.iter()
    .filter(|&&x| x > 0)
    .map(|&x| x * 2)
    .sum();

// ❌ 避免：临时变量过多
let mut result = 0;
for &x in &numbers {
    if x > 0 {
        result += x * 2;
    }
}
```

### 错误传播

```rust
// ✅ 推荐：使用 ? 操作符
fn read_file(path: &str) -> Result<String, io::Error> {
    let mut contents = String::new();
    File::open(path)?.read_to_string(&mut contents)?;
    Ok(contents)
}

// ❌ 避免：手动 match
fn read_file(path: &str) -> Result<String, io::Error> {
    let file = match File::open(path) {
        Ok(f) => f,
        Err(e) => return Err(e),
    };
    // ...
}
```

### Option 处理

```rust
// ✅ 推荐：使用组合子
let value = config.get("key").unwrap_or("default");
let doubled = number.map(|x| x * 2);

// ✅ 推荐：使用 if let
if let Some(user) = find_user(id) {
    process(user);
}
```

### 所有权转移与借用

```rust
// ✅ 推荐：不必要时不转移所有权
fn process(data: &[u8]) {
    // 使用切片，不获取所有权
}

// ✅ 推荐：需要修改时使用 &mut
fn update(list: &mut Vec<i32>) {
    list.push(42);
}
```

---

## 本地开发与 VSCode 环境

推荐读者使用本地 VSCode 环境进行代码实验。

### 推荐开发环境配置

| 组件 | 版本 | 说明 |
|------|------|------|
| Rust | 1.85+ | 通过 rustup 安装 |
| VSCode | 最新 | 免费编辑器 |
| rust-analyzer | 最新 | 必需插件 |
| CodeLLDB | 最新 | 调试支持 |

### VSCode 插件推荐

```json
{
  "recommendations": [
    "rust-lang.rust-analyzer",
    "vadimcn.vscode-lldb",
    "tamasfe.even-better-toml",
    "serayuzgur.crates"
  ]
}
```

### 章节代码实验指导

在章节末尾添加本地实验说明：

```markdown
### 本地实验

1. 创建新项目：`cargo new chapter_example`
2. 将代码复制到 `src/main.rs`
3. VSCode 中打开项目，rust-analyzer 会自动检查
4. 运行：`cargo run` 或点击 VSCode 的运行按钮
5. 尝试修改代码，观察编译器错误信息
```

### 快速创建测试项目

```bash
# 创建学习项目
cargo new rust-learning
cd rust-learning

# 运行示例
cargo run

# 运行 clippy 检查
cargo clippy

# 生成文档
cargo doc --open
```

---

## 本地实验项目规范

每章创建独立的 Cargo 项目，使用 `examples/` 目录组织所有示例代码。

### 目录结构

```
<章节目录>/
├── README.md                    # 章节导航
├── <小节文件>.md                # 理论内容
├── Cargo.toml                   # 章节项目配置
└── examples/
    ├── 01-<示例名称>.rs         # 对应第一小节
    ├── 02-<示例名称>.rs         # 对应第二小节
    └── ...
```

### Cargo.toml 模板

```toml
[package]
name = "chapter-<序号>-<主题>"
version = "0.1.0"
edition = "2024"

[dependencies]
# 按需添加依赖
```

**命名规则：**
- `name` 格式：`chapter-<两位序号>-<主题>`，如 `chapter-03-variables`
- `edition`：统一使用 `"2024"`

### 示例文件规范

#### 文件命名

- 格式：`<两位序号>-<示例名称>.rs`
- 示例名称使用 kebab-case（短横线命名）
- 序号与章节小节对应

#### 文件内容模板

```rust
//! # 示例：<示例标题>
//! 
//! 对应章节：<章节名称>
//! 运行：cargo run --example <文件名（不含.rs）>

fn main() {
    // ✅ 正确示例
    let x = 5;
    println!("x = {}", x);
    
    // 尝试修改：
    // 取消下面的注释观察编译错误：
    // x = 10;  // ❌ 不能修改不可变变量
}
```

#### 要求

1. 文件顶部必须有模块文档注释（`//!`）
2. 包含对应章节的 Markdown 说明
3. 正确示例使用 `// ✅` 标注
4. 错误示例注释掉并标注 `// ❌`
5. 鼓励读者修改代码实验

### 章节 README 更新

在每章 README.md 中添加"本地实验"段落：

```markdown
## 本地实验

本章示例代码位于 `examples/` 目录。

```bash
# 运行单个示例
cargo run --example 01-let-binding

# 编译检查所有示例
cargo check --examples
```
```

### 运行命令速查

| 命令 | 说明 |
|------|------|
| `cargo run --example <名称>` | 运行指定示例 |
| `cargo check --examples` | 编译检查所有示例 |
| `cargo clippy --examples` | lint 检查所有示例 |
| `cargo test --examples` | 运行示例中的测试 |

---

## 技术概念编写规范

每个技术概念必须包含：

| 重点 | 说明 | 必须 |
|------|------|------|
| **语法结构图** | ASCII 图展示语法组成，让读者一眼看懂 | ✅ 必须 |
| **最简示例** | 1-3 行展示核心用法 | ✅ 必须 |
| **详细示例** | 实际应用场景 | ✅ 必须 |
| **关键代码解释** | 核心/复杂代码需要单独说明 | ❓ 视情况 |
| **为什么用它** | 对比"有 vs 无"，展示价值 | ❓ 可选 |

"为什么用它"根据内容价值灵活决定：
- 新语法概念（如泛型、生命周期）→ 需要对比
- 已知概念的具体语法（如 match 语法）→ 不需要对比
- 替代方案（Vec vs 数组）→ 需要对比优劣
- 功能介绍（push 方法）→ 不需要对比

### 代码示例渐进复杂度

代码示例必须遵循渐进式学习路径：

```
最简示例（1-3 行） → 详细示例（实际应用） → 综合示例（完整场景）
```

| 阶段 | 目标 | 行数 | 内容 |
|------|------|------|------|
| 最简示例 | 展示核心语法 | 1-3 行 | 单一概念，无干扰代码 |
| 详细示例 | 展示实际应用 | 5-15 行 | 包含错误处理、边界情况 |
| 综合示例 | 完整场景 | 15+ 行 | 多个概念组合使用 |

---

### "为什么用它"对比规范

#### ✅ 需要对比的写法

```markdown
**切片 `&[T]`：**

`&[T]` 表示对数组或 Vec 的不可变借用。

```
语法结构：
┌──────────────────────────────────────┐
│  &[T]                                 │
│   ↑  ↑                                │
│   |  元素类型                         │
│   借用标记                             │
│                                       │
│  &[i32]   → 整数数组借用              │
│  &[String] → 字符串 Vec 借用          │
└──────────────────────────────────────┘
```

**为什么用它？**

```rust
// 没有：获取所有权，原数据不可再用
fn sum_vec(v: Vec<i32>) -> i32 {
    v.iter().sum()
}
let nums = vec![1, 2, 3];
let s = sum_vec(nums);
// nums 不能再使用

// 有：只借用，原数据保留
fn sum_slice(v: &[i32]) -> i32 {
    v.iter().sum()
}
let nums = vec![1, 2, 3];
let s = sum_slice(&nums);
// nums 仍然可用
```
```

#### ✅ 不需要对比的写法

```markdown
**`match` 表达式：**

`match` 用于模式匹配。

```
语法：
┌──────────────────────────────────────┐
│  match 值 {                           │
│      模式1 => 表达式1,               │
│      模式2 => 表达式2,               │
│      _ => 默认表达式,                │
│  }                                    │
│                                       │
│  必须穷尽所有可能                      │
└──────────────────────────────────────┘
```

### 最简示例

```rust
let number = 5;
match number {
    1 => println!("一"),
    2 | 3 | 5 => println!("质数"),
    _ => println!("其他"),
}
```
```

---

### 何时需要"为什么用它"

| 情况 | 是否需要 | 原因 |
|------|---------|------|
| 新语法概念（泛型、生命周期） | ✅ 需要 | 读者不理解价值 |
| 已知概念的语法（match 语法） | ❌ 不需要 | 读者已知道是什么 |
| 替代方案（Vec vs 数组） | ✅ 需要 | 需要对比优劣 |
| 功能介绍（Vec::push） | ❌ 不需要 | 功能显而易见 |
| Rust 特有概念（所有权、借用） | ✅ 需要 | 其他语言没有对应概念 |

---

### 不好写法 vs 好写法对比

展示常见反模式，帮助读者避免错误。

**❌ 不好的写法（缺少解释）：**

```rust
fn first_or_default<T: Default>(items: &[T]) -> T {
    items.first().cloned().unwrap_or_default()
}
```

问题：`&[T]` 是什么？`.cloned()` 为什么需要？读者不理解。

**✅ 好的写法（有解释）：**

```rust
fn first_or_default<T: Default>(items: &[T]) -> T {
    //                     ↑ 切片借用，不获取所有权
    //                                ↑ T: Default 约束
    items.first()             // 返回 Option<&T>
        .cloned()             // 将 &T 变为 T，需要 Clone
        .unwrap_or_default()  // None 时返回 T::default()
}
```

**关键代码说明：**

| 代码 | 含义 | 为什么这样写 |
|------|------|-------------|
| `&[T]` | 切片借用 | 不获取所有权，适用于 Vec 和数组 |
| `.first()` | 取第一个元素 | 返回 `Option<&T>`，安全访问 |
| `.cloned()` | 克隆元素 | 将 `&T` 变为 `T`，需要 `Clone` trait |
| `.unwrap_or_default()` | 空则返回默认值 | 需要 `Default` trait |

---

### 语法结构图示例

```markdown
**生命周期标注：**

```
语法结构：
┌──────────────────────────────────────┐
│  fn foo<'a>(x: &'a str) -> &'a str   │
│        ↑     ↑          ↑            │
│        声明  参数类型   返回类型      │
│                                       │
│  'a 必须在参数和返回值中一致使用       │
└──────────────────────────────────────┘
```
```

**泛型约束：**

```
语法结构：
┌──────────────────────────────────────┐
│  fn foo<T: Clone>(x: T) -> T         │
│         ↑          ↑                 │
│         类型参数   约束条件           │
│                                       │
│  T 必须实现 Clone trait               │
└──────────────────────────────────────┘
```
```

---

### 关键代码解释格式

**何时需要解释？**

| 情况 | 是否需要 | 处理方式 |
|------|---------|---------|
| 最简示例（1-3 行） | ❌ 不需要 | 代码本身已足够清晰 |
| 新语法概念（生命周期、泛型） | ✅ 必须解释 | 用表格或注释说明 |
| 所有权相关代码 | ✅ 必须解释 | 标注所有权转移点 |
| 复杂逻辑（多步骤） | ✅ 必须解释 | 分步骤注释 |
| 嵌套/组合结构 | ✅ 必须解释 | 拆解结构说明 |

**表格格式示例：**

```markdown
```rust
fn first_or_default<T: Default>(items: &[T]) -> T {
    items.first().cloned().unwrap_or_default()
}
```

**关键代码说明：**

| 代码片段 | 含义 | 为什么这样写 |
|----------|------|-------------|
| `&[T]` | 切片引用 | 不获取所有权，避免移动 |
| `.first()` | 取第一个元素 | 返回 `Option<&T>` |
| `.cloned()` | 克隆元素 | 将 `&T` 变为 `T`，需要 `Clone` |
| `unwrap_or_default()` | 空则返回默认值 | 需要 `Default` trait |
```

**注释格式示例：**

```markdown
```rust
fn process(s: &str) -> &str {
    // 第一步：查找边界
    // ↓ s 是借用，不获取所有权
    let bytes = s.as_bytes();
    
    // 第二步：处理数据
    // ↓ 返回值生命周期与 s 相同
    for (i, &item) in bytes.iter().enumerate() {
        if item == b' ' {
            return &s[0..i];  // 切片，所有权不变
        }
    }
    
    &s[..]
}
```
```

---

## 练习题格式

### 标准格式

```markdown
### 练习 X：题目名

**题目：** 详细描述

**难度：** 🟢 基础 / 🟡 进阶 / 🔴 挑战

```rust
// 代码模板或问题代码
```

<details>
<summary>查看答案</summary>

**答案：**

```rust
// 正确答案代码
```

**解释：**

答案和原理说明。

</details>

---
```

---

## 中文术语对照

保留英文原文对照：

| 中文 | 英文 | 使用场景 |
|------|------|----------|
| 所有权 | Ownership | 解释概念时用中文，代码注释用英文 |
| 借用 | Borrowing | 同上 |
| 生命周期 | Lifetime | 同上 |
| Trait | Trait | **保留英文** |
| Crate | Crate | **保留英文** |
| Module | Module | 可用"模块" |
| 泛型 | Generics | 可用中文 |
| 闭包 | Closure | 可用中文 |

**规则：**
- 正文中可使用中文术语
- 代码中保留英文术语
- 特殊术语（Trait, Crate）始终保留英文

---

## 检查清单

提交文档前检查：

### Markdown 格式
- [ ] 标题层级不超过三级
- [ ] 所有代码块标注语言
- [ ] 代码示例包含正确/错误标注
- [ ] **语法结构图是否存在？** ← 重点
- [ ] **是否有最简示例？** ← 重点
- [ ] **关键代码是否有解释？** ← 重点
- [ ] 链接有效且格式正确
- [ ] 表格格式对齐
- [ ] ASCII 艺术正确显示
- [ ] 包含练习题或练习题链接
- [ ] 添加小结部分
- [ ] 使用中文术语对照

### Rust 代码质量
- [ ] 代码通过 `cargo fmt` 格式化
- [ ] 代码通过 `cargo clippy` 检查（无 warning）
- [ ] 公共 API 包含 `///` 文档注释
- [ ] 错误处理使用 `Result` 而非 `panic`
- [ ] 优先使用 Iterator 链式调用而非 for 循环
- [ ] 优先使用 `?` 操作符而非手动 match
- [ ] 避免不必要的 `.clone()`
- [ ] 版本要求已标注（统一 `// 需要 Rust 1.85+`）

---

## 工具推荐

### Markdown 编辑器

- VS Code + Markdown Preview Enhanced
- Typora
- MarkText

### 格式检查

- markdownlint
- markdown-link-check

### ASCII 艺术

- asciiflow
- Monodraw

### Rust 工具链

- **cargo fmt** - 代码格式化（必须）
- **cargo clippy** - 代码 lint 检查（必须）
- **cargo doc** - 生成文档（第 28 章）
- **rust-analyzer** - IDE 支持
- **Rust Playground** - 在线代码实验

### 版本管理工具

- **cargo-msrv** - 验证最低 Rust 版本要求
- **rustup** - 多版本切换测试

---

## 版本标注规范

教程统一使用 Rust 1.85+ 作为最低版本要求。

### 版本标注位置

```rust
// 需要 Rust 1.85+
fn example() {
    // 使用 1.85 引入的特性
}
```

或在章节开头标注：

```markdown
> ⚠️ 需要 Rust 1.85+
```

### Edition 标注

统一使用 2024 Edition：

```rust
// edition = 2024
fn main() {
    // Rust 2024 Edition 特性
}
```

### 版本兼容性要求

- 所有代码示例应在 Rust 1.85+ 编译通过
- 使用 2024 Edition 特性
- 如使用不稳定特性，标注 `#![feature(...)]`
- `Cargo.toml` 中配置 `edition = "2024"`

---

## 参考资料

- [GitHub Markdown 指南](https://guides.github.com/features/mastering-markdown/)
- [Rust 文档风格指南](https://rust-lang.github.io/rfcs/1574-rustdoc-book.html)
- [中文技术文档写作指南](https://github.com/ruanyf/document-style-guide)

---

## 贡献风格改进

发现风格问题或有改进建议：

1. 在 Issues 中提出
2. 讨论并确定改进方案
3. 更新此文档

---

**更新日志：**
- 2026-04-15: 补充"为什么用它"对比规范、代码示例渐进复杂度、何时需要对比的判断规则、反模式展示规范
- 2026-04-15: 添加本地实验项目规范（examples/ 目录组织方式）
- 2026-04-15: 去掉 Playground 集成，改用本地 VSCode 环境；版本统一为 Rust 1.85+，使用 2024 Edition
- 2026-04-15: 添加 Rustdoc 文档注释规范、常见 Rust 惯用法、clippy 检查清单、版本标注规范
- 2026-04-11: 添加 Rust 学习策略（分层递进、场景驱动、对比学习、编译器当导师）
- 2026-04-11: 添加技术概念编写规范（语法结构图、关键代码解释）
- 2026-04-03: 创建风格指南